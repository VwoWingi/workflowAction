...
function incMajor(major: number): string {
  return `${major + 1}.0.0`;
}

/** Next minor boundary within a major, e.g. (1, 4) -> "1.5.0". */
function incMinor(major: number, minor: number): string {
 ...
 */
function rubyPessimisticToSemverRange(spec: string): string {
 ...
 */
function bracketRangeToSemverRange(spec: string): string | null {
  ...
  if (start) {
    const coerced = semver.coerce(start);
    if (!coerced) return null;
    pieces.push(`${startInclusive ? ">=" : ">"}` + coerced.version);
  }

  if (end) {
    const coerced = semver.coerce(end);
    if (!coerced) return null;
 ...
 */
function plusSuffixToSemverRange(spec: string): string | null {
  const trimmed = spec.trim();
 ...
 */
function isOutdated(currentSpec: string, latest: string | null): boolean {
  if (!latest) return false;

  const latestSemver = semver.coerce(latest);
  if (!latestSemver) return false;

  // Ruby-specific: translate `~>` or bare `~` (e.g. "~1.4") into Bundler-style semver ranges
  const isRubyTilde =
    currentSpec.includes("~>") || /^~\s*[0-9]/.test(currentSpec.trim());
  ...
  if (!currentSemver) return false;

  return semver.lt(currentSemver, latestSemver);
}
